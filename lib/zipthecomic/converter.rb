  ...
  class Converter
    ...
    def perform_conversion(source, dest_path)
      ...
      Dir.mktmpdir do |tmpdir|
        ...
        extracted_xml = File.join(tmpdir, "ComicInfo.xml")
        if File.exist?(extracted_xml)
          message = "Preserved existing ComicInfo.xml for #{base_name}"
        else
          File.write(extracted_xml, <<~XML)
            <?xml version="1.0"?>
            <ComicInfo>
          ...
          XML
          message = "Generated new ComicInfo.xml for #{base_name}"
        end

        @logger.info(message)

        if system("cd #{Shellwords.escape(tmpdir)} && zip -r -q #{Shellwords.escape(cbz_file)} .")
          puts message
          true
        else
          false
